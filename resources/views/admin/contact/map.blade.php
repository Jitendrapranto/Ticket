...
<body class="bg-[#F1F5F9] text-slate-800 font-plus">
    ...
    <div class="lg:ml-72 min-h-screen flex flex-col">
        ...
        <main class="p-8 flex-1">
            ...
            <form action="{{ route('admin.contact.map.update') }}" method="POST" enctype="multipart/form-data" class="max-w-4xl mx-auto bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                @csrf
                @method('PUT')
                ...
                <div class="p-10 space-y-10">
                    ...
                    <!-- Content Info -->
                    <div>
                        <h3 class="font-outfit text-lg font-black text-dark mb-6 border-b border-slate-100 pb-2 uppercase tracking-tighter"><i class="fas fa-info-circle mr-2 text-primary"></i> Map Section Info</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Section Title</label>
                                <input type="text" name="title" value="{{ old('title', $map->title) }}" class="w-full bg-slate-50 border border-slate-200 text-dark rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-bold">
                                @error('title') <p class="text-red-500 text-xs mt-1 font-bold">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Section Subtitle</label>
                                <input type="text" name="subtitle" value="{{ old('subtitle', $map->subtitle) }}" class="w-full bg-slate-50 border border-slate-200 text-dark rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-bold">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <h3 class="font-outfit text-lg font-black text-dark mb-6 border-b border-slate-100 pb-2 uppercase tracking-tighter"><i class="fas fa-map mr-2 text-primary"></i> Map Visual</h3>
                            
                            <div class="space-y-6">
                                <div>
                                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Background Image URL</label>
                                    <input type="text" name="map_image_url" value="{{ old('map_image_url', $map->map_image) }}" class="w-full bg-slate-50 border border-slate-200 text-dark rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all text-sm">
                                </div>

                                <div>
                                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Upload New Image</label>
                                    <input type="file" name="map_image_file" class="w-full bg-slate-50 border border-slate-200 text-dark rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all text-xs">
                                    @error('map_image_file') <p class="text-red-500 text-xs mt-1 font-bold">{{ $message }}</p> @enderror
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-[#F8FAFC] rounded-[2.5rem] p-10 border border-slate-100">
                        <h3 class="font-outfit text-lg font-black text-dark mb-6 border-b border-slate-200 pb-2 uppercase tracking-tighter"><i class="fas fa-location-dot mr-2 text-primary"></i> Interactive Map</h3>
