...
<head>
    ...
    <title>Edit Contact Card | Ticket Kinun Admin</title>
    ...
    <div class="lg:ml-72 min-h-screen flex flex-col">
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-8 sticky top-0 z-40">
            <div>
                ...
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest"><a href="{{ route('admin.contact.cards.index') }}" class="hover:text-primary">Contact Cards</a> / Edit</p>
            </div>
        </header>

        <main class="p-8 flex-1">
            <div class="max-w-2xl mx-auto bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-10">
                <form action="{{ route('admin.contact.cards.update', $card) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Title</label>
                        <input type="text" name="title" value="{{ old('title', $card->title) }}" required class="w-full bg-slate-50 border border-slate-200 text-dark rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-bold">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3" required class="w-full bg-slate-50 border border-slate-200 text-dark rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-medium text-sm">{{ old('description', $card->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="action_text" value="{{ old('action_text', $card->action_text) }}" class="w-full bg-slate-50 border border-slate-200 text-dark rounded-xl px-4 py-3 focus:outline-none transition-all font-bold text-sm">
                        ...
                        <div>
                            ...
                            <input type="text" name="action_url" value="{{ old('action_url', $card->action_url) }}" class="w-full bg-slate-50 border border-slate-200 text-dark rounded-xl px-4 py-3 focus:outline-none font-mono text-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="icon" value="{{ old('icon', $card->icon) }}" required class="w-full bg-slate-50 border border-slate-200 text-dark rounded-xl px-4 py-3 focus:outline-none font-mono text-sm">
                        ...
                        <div>
                            ...
                            <input type="number" name="sort_order" value="{{ old('sort_order', $card->sort_order) }}" class="w-full bg-slate-50 border border-slate-200 text-dark rounded-xl px-4 py-3 focus:outline-none font-mono text-sm">
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-50 p-6 rounded-2xl border border-slate-100 flex flex-col gap-6 mt-6">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div x-data="{ color: '{{ old('bg_color', $card->bg_color) }}' }">
                            ...
                            <div x-data="{ color: '{{ old('theme_color', $card->theme_color) }}' }">
                            ...
                            <div x-data="{ color: '{{ old('title_color', $card->title_color) }}' }">
                            ...
                            <div x-data="{ color: '{{ old('desc_color', $card->desc_color) }}' }">
