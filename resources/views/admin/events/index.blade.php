...
<head>
    ...
    <title>Manage Events | Ticket Kinun Admin</title>
...
<body class="bg-[#F1F5F9] text-slate-800 font-plus" x-data="{ 
    ...
    <div class="lg:ml-72 min-h-screen flex flex-col">
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-8 sticky top-0 z-40">
            <div>
                <h2 class="font-outfit text-xl font-black text-dark tracking-tight">Event Management</h2>
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Active Tickets & Listings</p>
            </div>
            <a href="{{ route('admin.events.create') }}" class="bg-accent text-white px-6 py-2.5 rounded-xl text-xs font-black tracking-widest hover:bg-dark transition-all uppercase shadow-lg shadow-accent/20">
                        ...
                        <div class="flex-1 text-left">
                            ...
                            <p class="text-[11px] text-red-200/60 mt-0.5 leading-tight">
                                @if(session('error'))
                                    {{ session('error') }}
                                @else
                                    Invalid data detected. Please check form fields.
                                @endif
                            </p>
                        </div>
                    </div>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="px-8 py-6">
                                    ...
                                    <p class="text-[10px] text-slate-400 font-medium truncate max-w-[150px]">{{ $event->location }}</p>
                                </td>
                                <td class="px-8 py-6 text-right">
                                    <div class="flex justify-end gap-3 items-center">
                                        <a href="{{ route('admin.events.edit', $event) }}" class="w-10 h-10 flex items-center justify-center rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-all shadow-sm">
