...
<head>
    ...
    <title>Manage Advantages | Ticket Kinun Admin</title>
...
<body class="bg-[#F1F5F9] text-slate-800 font-plus" x-data="{ 
    ...
    <div class="lg:ml-72 min-h-screen flex flex-col">
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-8 sticky top-0 z-40">
            <div>
                <h2 class="font-outfit text-xl font-black text-dark tracking-tight">Kinun Advantages</h2>
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Manage Advantage Cards</p>
            </div>
            <a href="{{ route('admin.about.advantages.create') }}" class="bg-accent text-white px-6 py-2.5 rounded-xl text-xs font-black tracking-widest hover:bg-dark transition-all uppercase shadow-lg shadow-accent/20">
                <i class="fas fa-plus mr-2"></i> Add Advantage
            </a>
        </header>
        ...
        <main class="p-8 flex-1">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @forelse($advantages as $advantage)
                    <div class="rounded-[2rem] p-10 shadow-sm border hover:shadow-md transition-all relative group {{ $advantage->border_class }}" style="background: {{ $advantage->card_bg_color }}">
                        
                        <div class="w-12 h-12 rounded-2xl flex items-center justify-center text-white mb-8 shadow-sm transition-transform group-hover:-translate-y-2" style="background: {{ $advantage->icon_bg_color }}">
                        ...
                        <p class="text-sm leading-relaxed font-medium" style="color: {{ $advantage->desc_color }}">
                            {{ Str::limit($advantage->description, 100) }}
                        </p>
                        <p class="text-[10px] text-gray-400 mt-4 font-mono">Order: {{ $advantage->sort_order }}</p>

                        <!-- Actions -->
                        <div class="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="{{ route('admin.about.advantages.edit', $advantage) }}" class="w-8 h-8 flex items-center justify-center rounded-lg bg-white/50 backdrop-blur text-blue-600 hover:bg-blue-600 hover:text-white transition-all shadow-sm border border-white/40">
                                <i class="fas fa-edit text-[10px]"></i>
                            </a>
                            <button @click="confirmDelete('{{ route('admin.about.advantages.destroy', $advantage) }}', '{{ $advantage->title }}')" class="w-8 h-8 flex items-center justify-center rounded-lg bg-white/50 backdrop-blur text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-sm border border-white/40">
                ...
                @empty
                    <div class="col-span-full py-12 text-center text-slate-400 font-bold bg-white rounded-3xl border border-dashed border-slate-200">
                        No advantage cards configured yet. Click "Add Advantage" to get started!
                    </div>
                @endforelse
            </div>
        </main>
            ...
            <div class="bg-white w-full max-w-[440px] rounded-[3rem] shadow-2xl relative z-10 overflow-hidden"
                ...
                <div class="p-12 text-center">
                    ...
                    <p class="text-slate-400 text-[15px] font-medium leading-relaxed mb-10 px-4">
                        You are about to delete the "<span class="text-dark font-black" x-text="itemName"></span>" advantage card. <br>This action cannot be undone.
                    </p>
