...
<head>
    ...
    <title>Manage About Statistics | Ticket Kinun Admin</title>
...
<body class="bg-[#F1F5F9] text-slate-800 font-plus" x-data="{ 
    ...
    <div class="lg:ml-72 min-h-screen flex flex-col">
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-8 sticky top-0 z-40">
            <div>
                <h2 class="font-outfit text-xl font-black text-dark tracking-tight">About Us Statistics</h2>
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Manage Statistic Cards</p>
            </div>
            <a href="{{ route('admin.about.statistics.create') }}" class="bg-accent text-white px-6 py-2.5 rounded-xl text-xs font-black tracking-widest hover:bg-dark transition-all uppercase shadow-lg shadow-accent/20">
        ...
        <main class="p-8 flex-1">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                @forelse($statistics as $stat)
                    <div class="bg-white rounded-[2rem] py-10 px-6 text-center flex flex-col items-center justify-center border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group">
                        <div class="absolute top-0 left-0 right-0 h-1.5" style="background:{{ $stat->color }}"></div>
                        <div class="w-14 h-14 rounded-[1rem] flex items-center justify-center text-white mb-6 transition-transform group-hover:scale-110" style="background:{{ $stat->color }}">
                        ...
                        <p class="text-[10px] text-gray-400 mt-2 font-mono">Order: {{ $stat->sort_order }}</p>

                        <!-- Actions -->
                        <div class="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="{{ route('admin.about.statistics.edit', $stat) }}" class="w-8 h-8 flex items-center justify-center rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-all shadow-sm">
                                <i class="fas fa-edit text-[10px]"></i>
                            </a>
                            <button @click="confirmDelete('{{ route('admin.about.statistics.destroy', $stat) }}', '{{ $stat->label }}')" class="w-8 h-8 flex items-center justify-center rounded-lg bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-sm">
                ...
                @empty
                    <div class="col-span-full py-12 text-center text-slate-400 font-bold bg-white rounded-3xl border border-dashed border-slate-200">
                        No statistics configured yet. Click "Add Statistic" to get started!
                    </div>
                @endforelse
            </div>
        </main>
            ...
            <div class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden"
                ...
                <div class="p-10 text-center">
                    ...
                    <p class="text-slate-400 text-sm font-medium leading-relaxed mb-8">
                        You are about to delete the <span class="text-dark font-bold" x-text="itemName"></span> statistic. This action cannot be undone.
                    </p>
