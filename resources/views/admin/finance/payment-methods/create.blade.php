...
<div class="px-8 py-8 animate-fadeIn">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-12 gap-6">
        <div>
            <div class="flex items-center gap-4 mb-3">
                ...
                <h2 class="font-outfit text-xl font-black text-dark tracking-tight">Create <span class="text-primary italic">Gateway</span></h2>
            </div>
            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest ml-14">Add a new manual payment method</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.finance.payment-methods.store') }}" method="POST" enctype="multipart/form-data" id="payment-form">
        @csrf
        <div class="bg-white border border-white rounded-[3rem] p-12 space-y-10 shadow-premium">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div class="space-y-4">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. bKash"
                ...
                <div class="space-y-4">
                    ...
                    <input type="text" name="account_number" value="{{ old('account_number') }}" placeholder="e.g. 017XX-XXXXXX"
            ...
            <div class="space-y-4">
                ...
                <textarea name="instructions" rows="6" required placeholder="Step 1: Go to your app...&#10;Step 2: Send money to our number..."
                          class="w-full bg-slate-50 border border-slate-100 rounded-[2rem] px-8 py-6 text-dark font-medium text-sm focus:border-primary focus:bg-white transition-all outline-none leading-relaxed">{{ old('instructions') }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div class="space-y-4">
                    ...
                    <div class="relative group border-2 border-dashed border-slate-100 rounded-[2.5rem] p-10 flex flex-col items-center justify-center hover:bg-slate-50 transition-all cursor-pointer">
                        ...
                        <i class="fas fa-image text-slate-200 text-3xl mb-4 group-hover:scale-110 transition-transform"></i>
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Upload Icon (.png, .jpg)</span>
                    </div>
                </div>

                <!-- QR Code Upload -->
                <div class="space-y-4">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Manual Payment QR Code</label>
                    <div class="relative group border-2 border-dashed border-slate-100 rounded-[2.5rem] p-10 flex flex-col items-center justify-center hover:bg-slate-50 transition-all cursor-pointer">
                        ...
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Upload Scanning QR</span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10 pt-6 border-t border-slate-50">
                ...
                <div class="space-y-4">
                    ...
                    <select name="is_active" class="w-full bg-slate-50 border border-slate-100 rounded-3xl px-8 py-5 text-dark font-bold text-sm focus:border-primary focus:bg-white transition-all outline-none italic appearance-none cursor-pointer shadow-sm">
                        <option value="1">Active (Published Online)</option>
                        <option value="0">Draft (Hidden from Checkout)</option>
                    </select>
                </div>
            ...
            <div class="pt-10 flex gap-4">
                <button type="button" onclick="confirmSave()" class="flex-1 bg-primary hover:bg-dark text-white py-6 rounded-[2rem] font-black text-xs tracking-[0.3em] uppercase transition-all shadow-premium active:scale-95 italic">
...
<script>
    function confirmSave() {
        ...
        Swal.fire({
            ...
            text: "This will create a new payment method for all users.",
            icon: 'question',
