...
<div x-data="{ preview: null }" x-ref="galleryForm">
            ...
            <header class="mb-8 flex items-center justify-between shrink-0">
                <div class="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Integrating new asset into the master gallery</p>
                    </div>
                </div>
                <div class="flex items-center gap-6">
                    ...
                    <button type="submit" class="bg-primary text-white px-10 py-4 rounded-2xl text-xs font-black tracking-widest hover:bg-primary-dark transition-all uppercase shadow-xl shadow-primary/20 flex items-center gap-4">
                        <i class="fas fa-plus text-[10px]"></i> Register Gallery
                    </button>
                </div>
            </header>
            ...
            <main class="p-12 max-w-5xl mx-auto space-y-12">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div class="space-y-10">
                            ...
                            <div class="space-y-10">
                                <div class="space-y-3 px-2">
                                    ...
                                    <div class="relative group">
                                        ...
                                        <input type="text" name="title" value="{{ old('title') }}" required class="w-full bg-white border border-slate-200/60 rounded-[1.5rem] py-5 pl-14 pr-8 outline-none focus:border-primary/40 focus:ring-4 focus:ring-primary/5 focus:bg-white transition-all text-dark font-bold text-sm shadow-sm hover:border-slate-300" placeholder="e.g. Neon Nights Symphony">
                                    </div>
                                </div>
                        ...
                        <div class="bg-white rounded-[3rem] p-10 shadow-premium border border-slate-50" x-data="{ enabled: {{ old('show_on_homepage') ? 'true' : 'false' }} }">
                            <div class="flex items-center justify-between">
                                ...
                                <button type="button" @click="enabled = !enabled" class="relative w-14 h-7 rounded-full transition-all duration-300 focus:outline-none" :class="enabled ? 'bg-primary shadow-lg shadow-primary/30' : 'bg-slate-200'">
                                    <span class="absolute top-0.5 left-0.5 w-6 h-6 bg-white rounded-full shadow-md transition-all duration-300" :class="enabled ? 'translate-x-7' : 'translate-x-0'"></span>
                                </button>
                            ...
                            <div x-show="enabled" x-collapse x-cloak class="mt-6 pt-6 border-t border-slate-100">
                                <div class="space-y-3 px-2">
                                    ...
                                    <div class="relative group">
                                        ...
                                        <input type="number" name="homepage_sort_order" value="{{ old('homepage_sort_order', 0) }}" min="0" class="w-full bg-white border border-slate-200/60 rounded-[1.5rem] py-5 pl-14 pr-8 outline-none focus:border-primary/40 focus:ring-4 focus:ring-primary/5 focus:bg-white transition-all text-dark font-bold text-sm shadow-sm hover:border-slate-300" placeholder="0">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-white rounded-[3rem] p-10 shadow-premium border border-slate-50 flex flex-col items-center justify-center text-center space-y-8 min-h-[500px]">
                        <input type="file" id="galleryImageInput" name="image" required class="hidden" accept="image/*">
                        ...
                        <div x-show="!preview" class="space-y-6 flex flex-col items-center">
                            ...
                            <button type="button" onclick="document.getElementById('galleryImageInput').click()" class="bg-primary text-white px-10 py-5 rounded-2xl font-black text-xs tracking-widest uppercase cursor-pointer hover:bg-black transition-all shadow-xl shadow-primary/10">
                        ...
                        <div x-show="preview" x-cloak class="w-full h-full relative group rounded-[2rem] overflow-hidden bg-slate-50 shadow-inner">
                            ...
                            <div class="absolute inset-0 bg-dark/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                <button type="button" onclick="document.getElementById('galleryImageInput').click()" class="bg-white text-dark px-8 py-4 rounded-xl font-black text-xs tracking-widest uppercase cursor-pointer hover:bg-primary hover:text-white transition-all">
...
<script data-swup-reload-script>
    (function() {
        ...
        if (!input || input._hasGalleryListener) return;
        input._hasGalleryListener = true;
        ...
        input.addEventListener('change', function(event) {
            ...
            if (file.size > maxSize) {
                if (window.Swal) {
                    ...
                    alert('File size too large: ' + (file.size / 1024).toFixed(2) + 'KB');
                }
                event.target.value = "";
                var el = document.querySelector('[x-ref="galleryForm"]');
                if (el && window.Alpine) {
                    if (window.Alpine.$data) {
                        window.Alpine.$data(el).preview = null;
                    } else if (el.__x) {
                        el.__x.$data.preview = null;
                    }
                }
                return;
            }
            ...
            reader.onload = function(e) {
                ...
                if (el && window.Alpine) {
                    if (window.Alpine.$data) {
                        ...
                        el.__x.$data.preview = e.target.result;
                    }
                }
            };
