        ...
        <form action="{{ route('admin.gallery.images.store') }}" method="POST" enctype="multipart/form-data" 
            ...
            <header class="h-24 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-12 sticky top-0 z-40">
                <div class="flex items-center gap-6">
                    <a href="{{ route('admin.gallery.images.index') }}" class="w-12 h-12 flex items-center justify-center rounded-2xl bg-slate-50 text-dark hover:bg-white transition-all border border-slate-100 shadow-sm italic">
                        <i class="fas fa-arrow-left text-xs"></i>
                    </a>
                    <div>
                        <h2 class="font-outfit text-2xl font-black text-dark tracking-tight italic">Enroll New Visual</h2>
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Adding to the visual exhibition</p>
                    </div>
                </div>
                <div class="flex items-center gap-6">
                    <a href="{{ route('admin.gallery.images.index') }}" class="text-xs font-black text-slate-400 hover:text-dark transition-colors uppercase tracking-[0.2em] italic mr-4">Discard</a>
                    <button type="submit" class="bg-primary text-white px-10 py-4 rounded-2xl text-xs font-black tracking-widest hover:bg-primary-dark transition-all uppercase shadow-xl shadow-primary/20 flex items-center gap-4 italic">
            ...
            <main class="p-12 max-w-5xl mx-auto space-y-12">
                @if($errors->any())
                    <div class="bg-red-50/50 border border-red-100 rounded-[2rem] p-8">
                        <div class="flex items-center gap-4 mb-4 text-red-500 italic">
                            <i class="fas fa-exclamation-circle text-xl"></i>
                            <h4 class="font-black text-sm uppercase tracking-widest">Registration Incomplete</h4>
                        </div>
                        <ul class="space-y-2">
                            @foreach ($errors->all() as $error)
                                <li class="text-[11px] text-red-400 font-bold italic flex items-center gap-3">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div class="space-y-10">
                        <div class="bg-white rounded-[3rem] p-10 shadow-premium border border-slate-50 space-y-8">
                            <div class="flex items-center gap-4 mb-4">
                                <div class="w-10 h-10 rounded-xl bg-primary/5 text-primary flex items-center justify-center italic">
                                    <i class="fas fa-info-circle"></i>
                                </div>
                                <h3 class="font-outfit text-lg font-black text-dark tracking-tight italic uppercase">Asset Identity</h3>
                            </div>
                                ...
                                <div class="space-y-3 px-2">
                                    ...
                                    <div class="relative group">
                                        ...
                                        <input type="text" 
                                               ...
                                               value="{{ old('title') }}" 
                                               required 
                                               class="w-full bg-white border border-slate-200/60 rounded-[1.5rem] py-5 pl-14 pr-8 outline-none focus:border-primary/40 focus:ring-4 focus:ring-primary/5 focus:bg-white transition-all text-dark font-bold text-sm italic shadow-sm hover:border-slate-300" 
                        ...
                        <div class="bg-dark rounded-[3rem] p-10 text-white relative overflow-hidden group border border-white/5 italic shadow-2xl">
                            ...
                            <p class="text-white/40 text-sm font-light leading-relaxed italic relative z-10">Selected visuals should reflect the high-energy and exclusive nature of the Ticket Kinun platform.</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-[3rem] p-10 shadow-premium border border-slate-50 flex flex-col items-center justify-center text-center space-y-8 min-h-[500px]">
                        ...
                        <div x-show="!preview" class="space-y-6 flex flex-col items-center">
                            <div class="w-24 h-24 bg-slate-50 rounded-[2rem] flex items-center justify-center text-slate-200 text-4xl shadow-inner italic animate-pulse">
                                <i class="fas fa-camera"></i>
                            </div>
                            <div>
                                <h3 class="font-outfit text-xl font-black text-dark tracking-tight italic uppercase">Capture the Essence</h3>
                                <p class="text-slate-400 text-sm font-medium mt-2">Recommended: 16:9 Aspect • Max 5MB</p>
                            </div>
                            <button type="button" @click="document.getElementById('imageInput').click()" class="bg-primary-dark text-white px-10 py-5 rounded-2xl font-black text-xs tracking-widest uppercase cursor-pointer hover:bg-black transition-all shadow-xl shadow-primary/10 italic">
                        ...
                        <div x-show="preview" x-cloak class="w-full h-full relative group rounded-[2rem] overflow-hidden bg-slate-50 shadow-inner">
                            ...
                            <div class="absolute inset-0 bg-dark/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                <button type="button" @click="document.getElementById('imageInput').click()" class="bg-white text-dark px-8 py-4 rounded-xl font-black text-xs tracking-widest uppercase cursor-pointer hover:bg-primary hover:text-white transition-all italic">
