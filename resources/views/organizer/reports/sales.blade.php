...
<head>
    ...
    <title>Sales Reports | Organizer Control Center</title>
...
<body class="bg-[#F8FAFC] text-slate-800 font-plus" x-data="{ searchQuery: '' }">
    ...
    <div class="lg:ml-72 min-h-screen flex flex-col">
        ...
        <main class="p-8 flex-1">
            ...
            <div class="flex flex-col xl:flex-row xl:items-end justify-between gap-6 mb-10">
                ...
                <form action="{{ route('organizer.reports.sales') }}" method="GET" class="flex flex-wrap items-center gap-3" id="filterForm">
                    ...
                    <a href="{{ route('organizer.reports.sales.export', ['start_date' => $startDate->format('Y-m-d'), 'end_date' => $endDate->format('Y-m-d')]) }}" class="bg-white text-dark border border-slate-100 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center gap-2">
                         <i class="fas fa-download"></i> Export Report
                    </a>
                    <button type="button" class="bg-secondary/5 text-secondary px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-secondary/10 transition-all flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                ...
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 relative overflow-hidden group">
                    <div class="flex items-start justify-between mb-6">
                        <p class="text-[10px] font-black tracking-widest text-slate-400 uppercase">Total Tickets Sold</p>
                    ...
                    <p class="text-[10px] font-bold text-brand-green flex items-center gap-1.5 uppercase tracking-widest">
                        <i class="fas fa-arrow-up"></i> +12.4% vs prev.
                    </p>
                </div>
                ...
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 relative overflow-hidden group">
                    ...
                    <p class="text-[10px] font-bold text-brand-green flex items-center gap-1.5 uppercase tracking-widest">
                        <i class="fas fa-arrow-up"></i> Influx peak hour
                    </p>
                </div>
                ...
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 relative overflow-hidden group">
                    <div class="flex items-start justify-between mb-6">
                        <p class="text-[10px] font-black tracking-widest text-slate-400 uppercase">Commission Deducted</p>
                    ...
                    <h3 class="font-outfit text-4xl font-black text-dark mb-1">৳{{ number_format($totalCommission, 2) }}</h3>
                ...
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 relative overflow-hidden group">
                    <div class="flex items-start justify-between mb-6">
                        <p class="text-[10px] font-black tracking-widest text-slate-400 uppercase">Net Profit</p>
                    ...
                    <h3 class="font-outfit text-4xl font-black text-dark mb-1">৳{{ number_format($netEarnings, 2) }}</h3>
                    <p class="text-[10px] font-bold text-brand-green flex items-center gap-1.5 uppercase tracking-widest">
                        Total earnings
                    </p>
                </div>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-8 mb-12">
                ...
                <div class="xl:col-span-8">
                    <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100">
                        <div class="flex flex-col sm:flex-row items-center justify-between mb-10 gap-4">
                            <div>
                                <h3 class="font-outfit text-xl font-black text-dark tracking-tight mb-2">Sales Trends</h3>
                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Visualizing volume fluctuations over time.</p>
                            </div>
                            <div class="bg-slate-50 p-1.5 rounded-2xl flex gap-1">
                                <button class="px-5 py-2 rounded-xl text-[9px] font-black uppercase text-slate-400 hover:text-dark transition-all">Daily</button>
                                <button class="px-5 py-2 rounded-xl text-[9px] font-black uppercase text-slate-400 hover:text-dark transition-all">Weekly</button>
                ...
                <div class="xl:col-span-4">
                    <div class="bg-secondary rounded-[3rem] p-12 text-white relative overflow-hidden shadow-2xl h-full flex flex-col justify-center items-center text-center group">
                        ...
                        <h4 class="font-outfit text-2xl font-black mb-4 tracking-tight">Platform Intelligence</h4>
                        <p class="text-xs font-medium leading-relaxed opacity-50 mb-10 max-w-[240px]">Analyze current ticket volume to generate growth recommendations and marketing strategies.</p>
                        <button class="w-full bg-white text-dark py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-100 hover:scale-[1.02] transition-all shadow-xl shadow-dark/20">Run Analysis Engine</button>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[3rem] shadow-premium border border-slate-50 overflow-hidden">
                <div class="p-10 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-slate-50/10">
                    <div>
                        <h3 class="font-outfit text-2xl font-black text-dark tracking-tighter mb-1">Event-Wise Ticket Sales</h3>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Performance breakdown by individual event.</p>
                    </div>
                    <button class="bg-white text-dark px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-slate-100 flex items-center gap-2 hover:bg-slate-50 transition-all">
                    ...
                    <table class="w-full text-left whitespace-nowrap">
                        <thead>
                            <tr class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] bg-white">
                                ...
                                <th class="px-10 py-6 text-right">Organizer Profit</th>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            ...
                            <tr class="group hover:bg-slate-50/50 transition-all"
                                ...
                                <td class="px-10 py-8">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{{ $event->category->name }} Event</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-10 py-8 text-right">
                                    <span class="px-3 py-1.5 rounded-full bg-emerald-50 text-brand-green text-[9px] font-black uppercase tracking-widest">+12%</span>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="5" class="px-10 py-24 text-center">
                                    <div class="w-24 h-24 bg-slate-50 rounded-[2rem] flex items-center justify-center mx-auto mb-6 text-slate-200 shadow-inner">
                                        <i class="fas fa-chart-line text-4xl"></i>
                                    </div>
                                    <h3 class="font-outfit text-xl font-black text-dark tracking-tight mb-2 uppercase">No Data Analysis</h3>
                                    <p class="text-slate-400 text-sm font-medium">We couldn't find any sales records for the selected period.</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
        ...
        <footer class="p-8 text-center text-[10px] font-black text-slate-400 tracking-widest uppercase border-t border-slate-50 bg-white">
            Ticket Kinun • Sales Intelligence Engine • © 2026
        </footer>
    </div>
