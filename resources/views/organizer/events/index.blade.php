...
<head>
    ...
    <title>Organizer Control Center | Ticket Kinun</title>
    ...
    <div class="lg:ml-72 min-h-screen flex flex-col">
        ...
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-8 sticky top-0 z-40">
            <div class="flex items-center gap-4">
                ...
                <div class="relative group hidden md:block">
                    ...
                    <input type="text" placeholder="Quick find event..." class="bg-slate-50 border border-slate-100 rounded-2xl pl-10 pr-6 py-2.5 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all w-64 uppercase tracking-tighter">
                </div>
            </div>
        ...
        <main class="p-8 flex-1">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                <div>
                    <h1 class="font-outfit text-4xl font-black text-dark tracking-tighter mb-2">My Control Center</h1>
                    <p class="text-slate-400 font-medium text-sm">Organize, track sales, and manage your live events.</p>
                </div>
                <div class="flex items-center gap-3">
                    <button class="bg-white text-dark px-6 py-3 rounded-2xl text-xs font-black tracking-widest uppercase border border-slate-100 flex items-center gap-2 hover:bg-slate-50 transition-all">
                        <i class="fas fa-download text-[10px]"></i> View Reports
                    </button>
                    <a href="{{ route('organizer.events.create') }}" class="bg-dark text-white px-6 py-3 rounded-2xl text-xs font-black tracking-widest uppercase flex items-center gap-2 hover:bg-primary transition-all shadow-xl shadow-dark/10">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                ...
                <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                    <p class="text-[10px] font-black tracking-widest text-slate-400 uppercase mb-3 text-center sm:text-left">Total Sales</p>
                    @php
                        $totalSoldOverall = 0;
                        foreach($events as $e) {
                    ...
                    @endphp
                    <h3 class="font-outfit text-4xl font-black text-primary text-center sm:text-left">{{ $totalSoldOverall }}</h3>
                </div>
            </div>
        ...
        <footer class="p-8 text-center text-[10px] font-black text-slate-400 tracking-widest uppercase border-t border-slate-50 bg-white">
            Ticket Kinun • Organizer Portal V1.2 • © 2026
        </footer>
    </div>
        ...
        <div class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl relative z-10 p-10 text-center">
            ...
            <div class="flex gap-4">
                ...
                <form :action="deleteUrl" method="POST" class="flex-1">
                    @csrf @method('DELETE')
                    <button type="submit" class="w-full py-4 rounded-2xl bg-red-500 text-white font-bold uppercase tracking-widest text-xs">Confirm Delete</button>
                </form>
            </div>
        </div>
    </div>
